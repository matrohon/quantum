...
class L2populationMechanismDriver(api.MechanismDriver,
    ...
    def delete_port_precommit(self, context):
        ...
        if context.current['status'] == const.PORT_STATUS_ACTIVE:
            ...
            self._fdb_to_del = self._build_fanout_msg_remove_fdb_entries(context)
    ...
    def _host_changed(self, context, orig, port):
        ...
        net_port_count_on_orig_agt = self.get_agent_net_port_up_count(
                                        ...
                                        orig_agent_host,
                                        network_id)
        net_port_count_on_new_agt = self.get_agent_net_port_up_count(
                                        session,
                                        port_agent_host,
                                        network_id)
        ...
        LOG.info(_("_host_changed : "
                   ...
                   "net_port_count_on_orig_agent : %(orig_agentcount)s;"
                   "net_port_count_on_new_agent : %(new_agentcount)s;"),
                 {'netcount' : net_port_count, 'orig_agentcount' : net_port_count_on_orig_agt, 
                  'new_agentcount' : net_port_count_on_new_agt})

        """orig_fdb_entries = self._get_fanout_fdb_entries_to_remove(
                                    orig,
                                    orig_agent_ip,
                                    orig_port_fdb_entries,
                                    net_port_count_on_orig_agt)
        """
        orig_fdb_entries = {orig_agent_ip:[]}

        if(net_port_count_on_orig_agt == 0):
            # Agent is removing its last port in this network,
            # other agents needs to be notified to delete their flooding entry.
            orig_fdb_entries[orig_agent_ip].append(
                const.FLOODING_ENTRY)

        orig_fdb_entries[orig_agent_ip] += orig_port_fdb_entries
        if (net_port_count - net_port_count_on_new_agt > 0 ):
            # there is port on other agent
            new_fdb_entries = self._get_fanout_fdb_entries_to_add(
                                                    port,
                                                    port_agent_ip,
                                                    port_fdb_entries,
                                                    net_port_count_on_new_agt)

            upd_fdb_entries = {network_id:
                               {'ports':
                                {'before': orig_fdb_entries,
                                 'after': new_fdb_entries
                                 }}}
            l2pop_rpc.L2populationAgentNotify.update_fdb_entries(
                self.rpc_ctx, {'chg_host': upd_fdb_entries})
            if net_port_count_on_new_agt == 1:
                # this is the first port on new agent 
                # and other port on other agent exist
                # cast it with fdb entries to other agent
                agent_fdb_entries = self._get_agent_fdb_entries(
                                        port_agent_host,
                                        network_id, segment)
                l2pop_rpc.L2populationAgentNotify.add_fdb_entries(
                    self.rpc_ctx, agent_fdb_entries, port_agent_host)
        elif (net_port_count_on_new_agt > 1):
            # new host already hosted a port of this net
            # cast it to remove fdb entries to the previous host
            cast_msg = {network_id:
                         {'segment_id': segment['segmentation_id'],
                          'network_type': segment['network_type'],
                          'ports': orig_fdb_entries}}
            l2pop_rpc.L2populationAgentNotify.remove_fdb_entries(
                self.rpc_ctx, cast_msg, port_agent_host)
    ...
    def _get_fanout_fdb_entries_to_add(self, port_context, agent_ip,
                                       port_fdb_entries,
                                       net_port_count_on_agt):
        session = db_api.get_session()
    ...
    def _update_port_up(self, context):
        ...
        ports = self._get_fanout_fdb_entries_to_add(port_context,
                                                    agent_ip,
                                                    port_fdb_entries,
                                                    net_port_count_on_agt)
        # if other agents hosts a port on this network, notify them and notifiy
    ...
    def _get_fanout_fdb_entries_to_remove(self, port_context, agent_ip,
                                          port_fdb_entries,
                                          net_port_count_on_agent):
        port_status = port_context['status']
        fdb_entries = {agent_ip:[]}

        if((port_status == const.PORT_STATUS_ACTIVE and
    ...
    def _build_fanout_msg_remove_fdb_entries(self, context):
        port_context = context.current
        port_infos = self._get_port_infos(context, port_context)
        if not port_infos:
            return
        ...
        if ((net_port_count - net_port_count_on_agent) > 0 ):
            ports = self._get_fanout_fdb_entries_to_remove(port_context,
                                                           agent_ip,
                                                           port_fdb_entries,
                                                           net_port_count_on_agent)
            if ports:
        ...
        net_port_count = self.get_net_ports_up(session, network_id).count()
        LOG.info(_("_build_fanout_msg_remove_fdb_entries : "
                   "net_port_count %(netcount)s ;"
    ...
    def _update_port_down(self, context):
        ...
        fdb_entries = self._build_fanout_msg_remove_fdb_entries(context)
        if fdb_entries:

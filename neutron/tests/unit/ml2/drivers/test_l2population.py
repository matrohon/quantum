        ...
        with self.subnet(network=self._network) as subnet:
            ...
            with self.port(subnet=subnet,
                ...
                with self.port(subnet=subnet,
                    ...
                    #on onother agent
                    #assert that fanouty without flooding entries is sent
                ...
                with self.port(subnet=subnet, cidr='10.0.0.0/24',
                    ...
                    with self.port(subnet=subnet, cidr='10.0.0.0/24',
                        ...
                        self.mock_fanout.assert_any_call(
                            ...
                            upd_expected,
                            topic=self.fanout_topic)
                    # port 3 has been deleted
                    # migrate the second port to host3
                    # assert that a fanout is sent with only after info
                    # assert that a cast is sent to host3
                    self.mock_cast.reset_mock()
                    self.mock_fanout.reset_mock()

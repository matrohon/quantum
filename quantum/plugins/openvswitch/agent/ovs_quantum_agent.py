...
class OVSQuantumAgent(sg_rpc.SecurityGroupAgentRpcCallbackMixin):
    ...
    def net_add_endpoint(self, context, **kwargs):
        ...
        LOG.debug(_("net_add_endpoint with net_id %(net_id)s"
                    " and endpoint %(endpoint)s"),
                  {'net_id': net_id, 'endpoint': endpoint})
        ...
        if net_id in self.tun_br_netport_map:
            ...
            LOG.debug(_("updating the output flow for network id %(net_id)s "
                        "with segmentation id %(seg_id)s; "
                        "new tunnel endpoint is %(endpoint)s"),
            ...
            LOG.debug(_("creating the output flow for network id %(net_id)s "
                        "with segmentation id %(seg_id)s; "
                        "new tunnel endpoint is %(endpoint)s"),
                      {'net_id': net_id, 'seg_id': segmentation_id,
                       'endpoint': endpoint})
    ...
    def net_del_endpoint(self, context, **kwargs):
        ...
        endpoint = kwargs.get('endpoint')
        net_id = kwargs.get('net_id')
        LOG.debug(_("net_del_endpoint with net_id %(net_id)s"
                    " and endpoint %(endpoint)s"),
                  {'net_id': net_id, 'endpoint': endpoint})
    ...
    def provision_local_vlan(self, net_uuid, network_type, physical_network,
        ...
        if network_type == constants.TYPE_GRE:
            if self.enable_tunneling:
                ...
                endpoints_ofports = []
                for i in endpoints:
                    LOG.debug(_("endpoints for net-uuid %(net_uuid)s "
                                "with segmentation id %(seg_id)s : %(i)s "),
                              {'net_uuid': net_uuid, 'seg_id': segmentation_id,
                              'i': i})
                    port_name = "gre-%s" % i
